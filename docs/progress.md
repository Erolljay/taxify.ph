...
- **Billing: one monthly invoice, high-water mark.** A business is billed for any month it was
  ...
  paying ₱0. The model deliberately avoids proration — it recalculates one monthly invoice on the
  high-water mark each cycle. (This was verified as the only clean path on PayMongo, which cannot
  prorate at all; after the 2026-07-22 switch to Xendit, whether Xendit prorates or needs the same
  recalculated-invoice approach must be re-verified against Xendit's docs before Phase 3 build.)
  Annual prepay deferred.
